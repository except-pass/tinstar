...
AMENDED AT U3: the per-space topology revision is a PERSISTED MONOTONIC COUNTER stored in the sidecar snapshot alongside the records, not reconstructed from them. The original wording had it reconstruct exactly as the maximum `homeRev` in the space, which was sound on the premise that no record is ever erased — KTD15 had made deletion a move. `purge`, added in U3 under that same KTD15, erases, and the premise expired with it: a derived revision runs BACKWARDS across a purge, which costs `expectedTopologyRev` both properties a compare-and-swap token needs, and an ABA was reproduced from five ordinary operations with no timing luck. The decision is corrected rather than quietly contradicted. `max(homeRev)` survives as the reload FLOOR for a snapshot written before the counter existed, so nothing already on disk faults or regresses, and the counter is allocated at commit time rather than at plan time so two concurrent mutations cannot claim the same number. Parent-child indexes are still reconstructed exactly from the records; only the revision moved.
