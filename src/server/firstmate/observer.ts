...
export class FirstmateObserver {
  ...
  private async project(entry: HomeEntry, task: string): Promise<void> {
    ...
    this.upsert(entry, runId, w, meta, terminal)
  }
  ...
  onTerminalExit(runId: string): void {
    ...
    if (!o || !entry) return
    void this.project(entry, o.task).catch(err => log.warn('firstmate', `terminal restart failed for ${o.task}: ${(err as Error).message}`))
  }
  ...
  private upsert(entry: HomeEntry, runId: string, w: WorkerState, meta: TaskMeta | null, terminal: TerminalResult): void {
