...
export const SLATE_AUTHOR_CONTRACT = [
  ...
  '  "group": "<optional set-id>",   // give the SAME group to a set of related QUESTIONS (2+) and they render',
  '                                  // side-by-side as a workbench, one question per column, each answered on its',
  '                                  // own. Omit it for a normal row. One question per entry — never bundle several',
  '                                  // questions into one entry, or they share a single answer box.',
  ...
  'ASKING THE READER SOMETHING (only when the surface is a QUESTION — otherwise omit these three entirely):',
