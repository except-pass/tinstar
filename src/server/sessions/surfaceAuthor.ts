...
export const SLATE_AUTHOR_CONTRACT = [
  ...
  '  "group": "<optional set-id>",   // give the SAME group',
  ...
  '  "refreshPolicy": {   // OPTIONAL (plan U6). Absent = the host decides which source drift marks it dirty.',
  '                       // Reading or selecting a dirty surface NEVER executes an agent recipe; the human',
  '                       // must explicitly use its refresh control.',
  '    "policy": "automatic" | "mark-stale" | "manual",   // automatic may run HOST recipes; agent recipes',
  '                                                        // still wait for explicit refresh; mark-stale only badges',
  ...
  'ASKING THE READER SOMETHING — only when the Surface IS a question. Direct files project as open points, and',
  'host-assigned compose cards are the exact file/id/token destinations named in the request; controls submit',
  'normally on both. `anchor` is legacy input and does not select a card layout. Otherwise omit these controls:',
  ...
  'DECISION AUTHORING: produce ONE Surface per human decision. Do not bundle unrelated signals or create one card',
  'per transcript turn. Work already owned by another agent or team is status/FYI, not an approval request.',
  'State verified facts with their source or observation time; label uncertain claims as hypotheses. Never turn an',
  'unverified alert into a leading choice. Options are likely paths, not an exhaustive constraint: the comment may',
  'supply another valid outcome such as delegation or waiting. NEVER put a refresh recipe on an unanswered Decision',
  'because regenerating it could rewrite the question while the human is answering.',
