...
function run(id: string): Run {
  // applyDelta's run-removed branch only reads id/marshal, so a thin shape is
  // enough here; cast keeps the fixture from tracking the full Run interface.
...
describe('applyDelta — run-removed prunes hidden-runs', () => {
  beforeEach(() => {
    localStorage.clear()
  })
  ...
  it('does not throw and does not prune on a marshal delete', () => {
    localStorage.setItem(LS_KEY, JSON.stringify(['marshal']))
    const prev = state({ marshal: run('marshal') })

    expect(() => applyDelta(prev, { entity: 'run', id: 'marshal', data: null })).not.toThrow()
    // Marshal isn't user-hideable; the marshal branch returns before pruning.
    expect(stored()).toContain('marshal')
  })
  ...
  it('is a safe no-op when the removed id is not hidden', () => {
    ...
    applyDelta(prev, { entity: 'run', id: 'dj', data: null })
