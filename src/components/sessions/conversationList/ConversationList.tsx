...
import { useSidechain } from "@/hooks/sessions/useSidechain";
...
export const ConversationList: FC<ConversationListProps> = ({
  ...
  useEffect(() => {
    if (validConversations.length > lastScannedLength) {
      const newPositions = new Set(responsePositions);

      // Only scan the new conversations (efficient!)
      for (let i = lastScannedLength; i < validConversations.length; i++) {
      ...
      setLastScannedLength(validConversations.length);
    }
  }, [
  ...
  const groupedConversations = useMemo(() => {
    ...
    for (let i = validConversations.length - 1; i >= 0; i--) {
      const conv = validConversations[i];
      ...
      if (conversation.type === "user") {
        ...
        if (!isToolResultOnly) {
          ...
          if (currentAssistantGroup.length > 0 && hasSeenUserMessage) {
            const hasAssistantMessage = currentAssistantGroup.some(
              (conv) => conv.type === "assistant",
            );
    ...
    if (currentAssistantGroup.length > 0 && hasSeenUserMessage) {
      // Only flush if the group contains at least one assistant message
      const hasAssistantMessage = currentAssistantGroup.some(
        (conv) => conv.type === "assistant",
      );
    ...
    return groups;
  }, [validConversations, responsePositions, conversations.length]);
    ...
    for (let i = group.conversations.length - 1; i >= 0; i--) {
      ...
      if (conversation && conversation.type === "assistant") {
        const textContent = conversation.message.content.find(
          (content: AssistantMessageContent) => content.type === "text",
        );
        if (
          textContent &&
          "text" in textContent &&
